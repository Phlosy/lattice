...
export function createLatticeRemote(opts: RemoteRuntimeOptions): LatticeApi {
  ...
  function connect() {
    ...
    ws.onopen = () => {
      ...
      if (opts.token) sendRaw({ type: "auth", token: opts.token });
    };
    ws.onmessage = (e) => {
      let msg: any;
      try {
  ...
  return {
    appInfo: () => Promise.resolve({ name: "Lattice", version: "1.1.0", platform: "mobile" }),
    ...
    prompt: (sessionId, text) => request("prompt", { sessionId, text }),
