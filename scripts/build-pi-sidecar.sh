...
# The Pi executable is nested code inside Lattice.app. Sign it before Tauri
# signs the containing app so the final bundle has a valid inside-out signature.
# APPLE_SIGNING_IDENTITY is set to a Developer ID identity by the release
# workflow in developer-id mode; otherwise macOS builds default to ad-hoc (`-`).
if [[ "$PLATFORM" == darwin-* ]]; then
  command -v codesign >/dev/null 2>&1 || {
    echo "error: codesign is required for macOS sidecar builds" >&2
    exit 1
  }
